...
class ArtikelController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('gambar')) {
            ...
            $filePath = $file->storeAs('upload/artikel', $filename, 'public');
    ...
    public function update(Request $request, Artikel $artikel)
    {
        ...
        if ($request->hasFile('gambar')) {
            if (Storage::disk('public')->exists('upload/artikel' . $artikel->gambar)) {
                Storage::disk('public')->delete('upload/artikel' . $artikel->gambar);
            }

            $file = $request->file('gambar');
            $filename = time() . '.' . $file->getClientOriginalExtension();

            $file->storeAs('upload/artikel', $filename, 'public');
