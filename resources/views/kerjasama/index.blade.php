                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="/admin/kerjasama"
                            class="ms-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ms-2 dark:text-gray-400 dark:hover:text-white">kerjasama</a>
                    </div>
                </li>
            </ol>
        </nav>
        <h3 class="text-3xl font-bold dark:text-white px-4 lg:px-12">Kerjasama</h3>
                ...
                <div class="flex items-center w-full">
                    <form action="{{ route('artikel.search') }}" method="GET"
                        ...
                        <div class="relative flex-grow">
                            ...
                            <input type="text" name="query" placeholder="Cari kerjasama..."
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        ...
                        <tr class="border-b dark:border-gray-700">
                            ...
                            <td class="px-4 py-3">
                                {{ ($kerjasamas->currentPage() - 1) * $kerjasamas->perPage() + $loop->iteration }}</td>
                            <td class="px-4 py-3">{{ $kerjasama->nama_instansi }}</td>
                            <td class="px-4 py-3">
                                <img src="{{ asset('storage/upload/kerjasama/' . $kerjasama->gambar) }}"
                                    alt="Product Image" class="w-16 md:w-24">
                            </td>
                            <td class="px-4 py-3 flex items-center">
                                <button id="actionsDropdownButton-{{ $kerjasama->id }}"
                                    data-dropdown-toggle="actionsDropdown-{{ $kerjasama->id }}"
                                ...
                                <div id="actionsDropdown-{{ $kerjasama->id }}"
                                    class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                    <ul class="py-1 text-sm text-gray-700 dark:text-gray-200"
                                        aria-labelledby="actionsDropdownButton-{{ $kerjasama->id }}">
                                        <li>
                                    ...
                                    <div class="py-1">
                                        <form action=" {{ route('kerjasama.destroy', $kerjasama->id) }}" method="POST"
                                            onsubmit="return confirm('Apakah anda yakin ingin menghapus kerjasama ini?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
            ...
            <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4"
                ...
                <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                    ...
                    <span class="font-semibold text-gray-900 dark:text-white">
                        @if ($kerjasamas->count())
                        {{ $kerjasamas->firstItem() }}-{{ $kerjasamas->lastItem() }}
                        @else
                        0-0
                        @endif
                    </span>
                    dari
                    <span class="font-semibold text-gray-900 dark:text-white">{{ $kerjasamas->total() }}</span>
                </span>

                <!-- Dynamic Pagination Links -->
                <div>
                    {{ $kerjasamas->links() }}
                    <!-- Menampilkan link navigasi halaman -->
                </div>
            </nav>
        </div>
    </x-layout>
