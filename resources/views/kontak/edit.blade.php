    ...
    <x-layout>
        ...
        <form action="{{ route('kontak.update', $kontak->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="nama" name="nama_aplikasi" value="{{ $kontak->nama_aplikasi }}"
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            </div>

            <div class="mb-4">
                <label for="bahan" class="block text-gray-700">Tautan:</label>
                <input type="text" id="bahan" placeholder="https://..." name="tautan" value="{{ $kontak->tautan }}"
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            </div>

            <div class="mb-4">
                <label for="image" class="block text-gray-700">Gambar:</label>
                <input type="file" id="image" name="gambar" value="{{ $kontak->gambar }}"
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                <img src="{{ asset('storage/upload/kontak/' . $kontak->gambar) }}" alt="Produk Image"
                    class="mt-2 w-36 h-36">
            </div>

            <div class="flex items-center justify-between">
                <button type="submit"
                    class="bg-yellow-400 hover:bg-yellow-500 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">Perbaharui
                    Kontak</button>
            </div>
        </form>
    </x-layout>
