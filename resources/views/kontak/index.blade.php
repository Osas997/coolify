            ...
            <ol class="inline-flex items-center space-x-3 md:space-x-2 rtl:space-x-reverse">
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="/admin/kontak"
                            class="ms-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ms-2 dark:text-gray-400 dark:hover:text-white">Kontak</a>
                    </div>
                </li>
            </ol>
        </nav>
        <h3 class="text-3xl font-bold dark:text-white px-4 lg:px-12">Kontak</h3>
                ...
                <div class="flex items-center w-full">
                    <form action="{{ route('artikel.search') }}" method="GET"
                        ...
                        <div class="relative flex-grow">
                            ...
                            <input type="text" name="query" placeholder="Cari Kontak..."
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        ...
                        <tr class="border-b dark:border-gray-700">
                            ...
                            <td class="px-4 py-3">
                                {{ ($kontaks->currentPage() - 1) * $kontaks->perPage() + $loop->iteration }}</td>
                            ...
                            <td class="px-4 py-3 flex items-center">
                                <button id="actionsDropdownButton-{{ $kontak->id }}"
                                    data-dropdown-toggle="actionsDropdown-{{ $kontak->id }}"
                                ...
                                <div id="actionsDropdown-{{ $kontak->id }}"
                                    class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                    <ul class="py-1 text-sm text-gray-700 dark:text-gray-200"
                                        aria-labelledby="actionsDropdownButton-{{ $kontak->id }}">
                                        <li>
            ...
            <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4"
                ...
                <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                    ...
                    <span class="font-semibold text-gray-900 dark:text-white">
                        @if ($kontaks->count())
                        {{ $kontaks->firstItem() }}-{{ $kontaks->lastItem() }}
                        @else
                        0-0
                        @endif
                    </span>
                    dari
                    <span class="font-semibold text-gray-900 dark:text-white">{{ $kontaks->total() }}</span>
                </span>
